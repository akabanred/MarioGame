...
{
	{
		...
		AnimationCache::getInstance()->addAnimation(myutil::createAnimation("smallWalkLeft.png",
																			0, 10, 14, 0.05f),
													"smallMoveLeftAnimation");
		AnimationCache::getInstance()->addAnimation(myutil::createAnimation("smallWalkRight.png",
																			0, 10, 14, 0.05f),
													"smallMoveRightAnimation");

		// ���������ʱ��Ҫ�õ�֡
		SpriteFrameCache::getInstance()
			->addSpriteFrame(myutil::getSpriteFrame("smallWalkLeft.png", 10, 14),
							 "smallJumpLeft");
		SpriteFrameCache::getInstance()->addSpriteFrame(myutil::getSpriteFrame("smallWalkRight.png", 10, 14), "smallJumpRight");
	}
	{
	...
	if (_isFly)
	{
		if (_state != State::Small)
		{
			this->setSpriteFrame(SpriteFrameCache::getInstance()
									 ->getSpriteFrameByName(_faceDir == common::LEFT ? "bigJumpLeft" : "bigJumpRight"));
		}
		else
		{
			this->setSpriteFrame(SpriteFrameCache::getInstance()
									 ->getSpriteFrameByName(_faceDir == common::LEFT ? "smallJumpLeft" : "smallJumpRight"));
		}
...
void Mario::eatMushroom(Item::ItemType type)
{
	if (type == Item::IT_MUSHROOMREWARD)
		_state = State::Big;
	AudioEngine::play2d(SOUND_eat_mushroom_or_flower);
	updateStatus();
}
...
void Mario::fireArrow()
{
	...
	for (auto n : parent->getChildren())
		if (auto b = dynamic_cast<ItemBullet *>(n))
			if (b->getOwnerCategory() == CAT_MARIO)
				active++;
	if (active >= 2)
		return;
	...
	AudioEngine::play2d(SOUND_bullet);
}
